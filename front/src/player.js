...
class Player {
  ...
  collisionWithFireCheck() {
    ...
    if (firesTouchingMe) {
      ...
      firesTouchingMe.directionRight
        ? this.setDx(FIRE_FORCE)
        : this.setDx(-FIRE_FORCE);
    }
  }
  ...
  jump() {
    if (this.onGround) {
      // check if you're on the ground before jumping
  ...
  keyDownHandler(e) {
    if (this.controls.right.includes(e.key)) {
      ...
      this.fireManager.fire({
        x: this.x,
        y: this.y,
        directionRight: this.directionRight,
      });
    }
  }
